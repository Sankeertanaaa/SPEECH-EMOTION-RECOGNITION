...
text_emotion_classifier = pipeline("sentiment-analysis", model="dair-ai/emotion", device=0 if device=="cuda" else -1)
...
def main(audio_path, patient_name="N/A", patient_age="N/A", patient_gender="N/A"):
    try:
        ...
        # Get text emotions
        text_emotions = analyze_text_emotions(transcript)
        print(f"Detected text emotions: {text_emotions}", file=sys.stderr)
